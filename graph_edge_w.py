...
import numpy as np
from graph_representation_w import graph_vertices,directed_decision


def insert_edge(mat,from_vertex,to_vertex):
	print("Enter from_vertex and to_vertex and weight: ")
	...
	if(from_vertex in graph_vertices and to_vertex in graph_vertices):
		if(mat[from_vertex-1,to_vertex-1]==0):
			mat[from_vertex-1,to_vertex-1]=weight
			if(directed_decision==0):
				mat[to_vertex-1,from_vertex-1]=weight
		else:
			mat[from_vertex-1,to_vertex-1]=weight
			if(directed_decision==0):
				mat[to_vertex-1,from_vertex-1]=weight			
...
def delete_edge(mat,from_vertex,to_vertex):
	...
	if(from_vertex in graph_vertices and to_vertex in graph_vertices):
		if(mat[from_vertex-1,to_vertex-1]==1):
			...
			if(directed_decision==0):
				mat[to_vertex-1,from_vertex-1]=0
			print("Edge deleted")
		else:
